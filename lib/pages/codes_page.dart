...
class _QRGeneratorPageState extends State<QRGeneratorPage> {
  ...
  Future<Uint8List> generateQrCodeWithLabel(String code) async {
    const int qrSize = 420;   // más grande → mejor legibilidad / velocidad de escaneo
    const int margin = 28;    // zona de silencio (quiet zone) alrededor del QR
    const int topH = 52;      // prefijo
    const int bottomH = 96;   // lote + secuencia
    final int w = qrSize + margin * 2;
    final int h = topH + qrSize + margin * 2 + bottomH;
    ...
    final prefijo = partes.length > 1 ? partes.first : '';
    final serie = partes.last;

    // prefijo (arriba) — también forma parte de la información codificada en el QR
    if (prefijo.isNotEmpty) {
      final tpPrefijo = TextPainter(
        text: TextSpan(
          text: prefijo,
      ...
      )..layout(minWidth: w.toDouble(), maxWidth: w.toDouble());
      tpPrefijo.paint(canvas, const Offset(0, 10));
    }

    // QR con mayor corrección de errores y zona de silencio
    final qrCode = QrValidator.validate(
      data: code,
    ...
    canvas.save();
    canvas.translate(margin.toDouble(), (topH + margin).toDouble());
    QrPainter.withQr(
      qr: qrCode,
      color: Colors.black,
    ...
    // lote + secuencia (abajo, grande)
    final tpSerie = TextPainter(
      text: TextSpan(
        text: serie,
        style: const TextStyle(
          color: Colors.black,
    ...
    tpSerie.paint(canvas, Offset(0, (topH + qrSize + margin * 2 + 6).toDouble()));
  ...
  Future<void> _generarCodigos() async {
    ...
    // Determinar el siguiente número de secuencia consultando los existentes
    int siguienteSecuencia = 1;
    try {
      final existentes = await ApiService().obtenerCodigosQR();
      final patron = '$prefix-$lote';
      final numerosExistentes = existentes
          .map((c) => c['codigo'] as String)
          .where((c) => c.startsWith(patron))
          .map((c) => int.tryParse(c.substring(patron.length)) ?? 0)
          .toList();
      if (numerosExistentes.isNotEmpty) {
        siguienteSecuencia = numerosExistentes.reduce((a, b) => a > b ? a : b) + 1;
      }
    } catch (_) {
      // Si falla la consulta, comenzar desde 1
    }

    final codigos = List.generate(
      cantidad,
      (i) => '$prefix-$lote${(siguienteSecuencia + i).toString().padLeft(3, '0')}',
    );

    final archive = Archive();
    for (final codigo in codigos) {
      try {
        // Guardar primero en BD; si falla (ej. duplicado), no incluir en ZIP
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Omitido $codigo: ya existe o error al guardar')),
          );
        }
      }
    }
